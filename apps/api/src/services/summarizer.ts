...
const SYSTEM_PROMPT = `You are a-RSS Summarizer. You read a single news article and rewrite it as one short introductory sentence followed by exactly three concise bullets that a busy reader can scan in seconds.
...
async function callOnce(input: SummarizeInput): Promise<SummarizeResult> {
  const c = buildClient(input.apiKey);
  ...
  if (!textBlock || textBlock.type !== 'text') {
    throw new Error('summarizer returned no text block');
  }
  // The model may rarely wrap output in markdown code fences despite instructions; strip them.
  const raw = textBlock.text.trim().replace(/^```json\s*/i, '').replace(/```$/, '').trim();
  let parsedJson: unknown;
  try {
    parsedJson = JSON.parse(raw);
  } catch (err) {
    throw new Error(`summarizer output was not valid JSON: ${(err as Error).message}`);
  }
  const validated = responseSchema.parse(parsedJson);
  return {
    intro: validated.intro,
    bullets: validated.bullets as [string, string, string],
    model: env.SUMMARIZER_MODEL,
  };
}
